...
bool buffer_test() {
	...
	if ( yas::is_binary_archive<OA>::value ){
		...
		if ( strncmp(str, "yas91 21 intrusive buffer test", oa1.get_intrusive_buffer().size) ) {
			std::cout << "BUFFER intrusive serialization error! [1]" << std::endl;
			return false;
		}
	}
	...
	if ( yas::is_binary_archive<OA>::value ){
		...
		{
			std::cout << "BUFFER std shared buffer serialization error! [2]" << std::endl;
			return false;
		}
	} else if ( yas::is_text_archive<OA>::value ) {
		const char* str = static_cast<const char*>(oa2.get_intrusive_buffer().data);
		if ( strncmp(str, "yas91 22 std shared buffer test", oa2.get_intrusive_buffer().size) ) {
			std::cout << "BUFFER std shared buffer serialization error! [2]" << std::endl;
			return false;
		}
	}
	...
	if ( buf4.size != str2.length() || str2 != buf4.data.get() ) {
		std::cout << "BUFFER std shared buffer deserialization error! [3]" << std::endl;
		return false;
	}
#endif
	...
	if ( yas::is_binary_archive<OA>::value ){
		...
		{
			std::cout << "BUFFER boost shared buffer serialization error! [4]" << std::endl;
			return false;
		}
	} else if ( yas::is_text_archive<OA>::value ) {
		const char* str = static_cast<const char*>(oa3.get_intrusive_buffer().data);
		if ( strncmp(str, "yas91 24 boost shared buffer test", oa3.get_intrusive_buffer().size) ) {
			std::cout << "BUFFER boost shared buffer serialization error! [4]" << std::endl;
			return false;
		}
	}
	...
	if ( buf5.size != str3.length() || str3 != buf5.data.get() ) {
		std::cout << "BUFFER boost shared buffer deserialization error! [5]" << std::endl;
		return false;
	}
#endif
