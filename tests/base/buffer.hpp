...
bool buffer_test() {
	...
	if ( yas::is_binary_archive<OA>::value ){
		if ( oa1.get_intrusive_buffer().size != sizeof(ostr1_64) )
		{
			std::cout << "BUFFER intrusive serialization error! [1]" << std::endl;
			return false;
		}
		if ( memcmp(oa1.get_intrusive_buffer().data, ostr1, oa1.get_intrusive_buffer().size) ) {
		...
		;
		const char* str = static_cast<const char*>(oa1.get_intrusive_buffer().data);
		if ( oa1.get_intrusive_buffer().size != strlen(res) ) {
			std::cout << "BUFFER intrusive serialization error! [3]" << std::endl;
			return false;
		}
		if ( strncmp(str, res, oa1.get_intrusive_buffer().size) ) {
	...
	oa2 & buf2;
	if ( yas::is_binary_archive<OA>::value ){
		if ( oa2.get_intrusive_buffer().size != sizeof(ostr2_64) )
		{
			std::cout << "BUFFER intrusive serialization error! [5]" << std::endl;
			return false;
		}
		if ( memcmp(oa2.get_intrusive_buffer().data, ostr2, oa2.get_intrusive_buffer().size) ) {
		...
		;
		const char* str = static_cast<const char*>(oa2.get_intrusive_buffer().data);
		if ( oa2.get_intrusive_buffer().size != strlen(res) ) {
			std::cout << "BUFFER intrusive serialization error! [7]" << std::endl;
			return false;
		}
		if ( strncmp(str, res, oa2.get_intrusive_buffer().size) ) {
	...
	oa3 & buf3;
	if ( yas::is_binary_archive<OA>::value ){
		if ( oa3.get_intrusive_buffer().size != sizeof(ostr3_64) )
		{
			std::cout << "BUFFER intrusive serialization error! [10]" << std::endl;
			return false;
		}
		if ( memcmp(oa3.get_intrusive_buffer().data, ostr3, oa3.get_intrusive_buffer().size) ) {
		...
		;
		const char* str = static_cast<const char*>(oa3.get_intrusive_buffer().data);
		if ( oa3.get_intrusive_buffer().size != strlen(res) ) {
			std::cout << "BUFFER intrusive serialization error! [12]" << std::endl;
			return false;
		}
		if ( strncmp(str, res, oa3.get_intrusive_buffer().size) ) {
