...
> {
    ...
    static Archive& save(Archive& ar, const std::string& string) {
        if ( F & yas::json ) {
            ...
            const std::size_t n = extra_space(string);
            // must be escaped
            if ( n ) {
                std::string escaped(string.length() + n, '\\');
...
private:
    ...
    static std::size_t extra_space(const std::basic_string<CharT> &s) {
        std::size_t res = 0;
        for ( const auto &c: s ) {
            switch ( c ) {
                case '"':
    ...
    template<typename CharT>
    static void escape(std::basic_string<CharT> &d, const std::basic_string<CharT> &s) {
        std::size_t pos = 0;
    ...
    template<typename Archive>
    static int get_codepoint(Archive &ar)  {
        int codepoint = 0;
    ...
    template<typename CharT, typename Archive>
    static void read_string(std::basic_string<CharT> &d, Archive &ar) {
            ...
            switch ( ch ) {
                ...
                case '\\': {
                    ...
                    switch ( ch2 ) {
                        ...
                        case 'u': {
                            int codepoint;
                            int codepoint1 = get_codepoint(ar);
                            ...
                            if ( 0xD800 <= codepoint1 && codepoint1 <= 0xDBFF ) {
                                ...
                                if ( JSON_LIKELY(get() == '\\' && get() == 'u') ) {
                                    const int codepoint2 = get_codepoint(ar);
