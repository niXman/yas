...
> {
	...
	static Archive& save(Archive &ar, const asis_wrapper<T> &v) {
	    __YAS_CONSTEXPR_IF( (F & yas::text) || (F & yas::json) ) {
            return (ar & v.val);
	    } else {
            ar.write(v);
	    }
	...
	template<typename Archive>
	static Archive& load(Archive &ar, asis_wrapper<T> &v) {
        __YAS_CONSTEXPR_IF( (F & yas::text) || (F & yas::json) ) {
            return (ar & v.val);
        } else {
            ar.read(v);
...
#endif // __yas__types__utility__asis_hpp
